...
zclass[[box,pos|
    rx,0, ry,0,
    touching,%f_box_touching,
    inside,%f_box_inside,
    outside,%f_box_outside,
    side,%f_box_side,
    abside,%f_box_abside,
    getdelta,%f_box_getdelta
]]

|[f_box_touching]| function(a, b)
    return not a:outside(b) and not a:inside(b)
end $$

|[f_box_outside]| function(a, b)
    local xp, yp, xr, yr = a:side(b)
    return xp <= -1-xr or xp >= 1+xr or yp <= -1-yr or yp >= 1+yr
end $$

|[f_box_inside]| function(a, b)
    local xp, yp, xr, yr = a:side(b)
    return xp > -1+xr and xp < 1-xr and yp > -1+yr and yp < 1-yr
end $$

|[f_box_side]| function(a, b)
...
|[f_box_getdelta]| function(a, b, dx, dy)
