...
for i=1,252 do -- There are 252 pkmn and 252 moves. So zipped when unpacking to save some tokens.
  local pkmn = f_zobj([[moves_progress;,#,#,#; moves_grouped;,#,#,#; lock,~c_no, name,@, num,@]], c_pkmn_names[i], i%252) -- missingno is 252

  -- cur_list is not local just so I can save 1 token
  cur_list, c_moves[i-1], c_pokemon[i] = pkmn.moves_progress[1], f_zobj([[lock,~c_no, num,@, name,@]], i-1, c_move_names[i-1]), pkmn -- todo: get rid of i-1
...
for i=1,252 do -- todo: token crunching - can move up
  -- without setting this as a function, and using metatables,
  -- i run out of lua memory space.
