...
end,function(game)f_print_draw3_message(_split(game:entry"main".desc,"|"))end,function(_ENV)
f_print_draw3_message{
name,
hp.."/"..maxhp,
"paralyzed"
}
end,function(_ENV)f_fight_draw3_helper(p0.active)end,function(_ENV)f_fight_draw3_helper(_ENV:entry"pselswitch".pkmn)end,function(game)
...
menu:refresh(f_zobj[[;name,fight,select,%f_menu_state_callback,state,pselmove;;name,random,select,%f_modes_pop;;name,switch,select,%f_menu_state_callback,state,pselswitch;;name,forfeit,select,%f_psel_forfeit]])
end,function(_ENV)
