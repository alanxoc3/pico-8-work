...
f_zobj_set(_g,"f_zobj,@,f_zobj_set,@,f_zobj_eval,@,f_zclass,@,f_zclass_register,@,f_zclass_kill,@,f_zclass_loop,@,f_zclass_clean,@,f_zclass_get_elapsed_percent,@,f_zclass_load,@,f_zclass_loadlogic,@,f_zclass_state,@,f_draw_pixelgroup,@,f_pixelgroup_set_pixel,@,f_pixelgroup_get_pixel,@,f_pixelgroup_push,@,f_pixelgroup_combine_moved,@,f_pixelgroup_check,@,f_pixelgroup_combine,@,f_create_pixelgroup,@,f_pixelgroup_set_dir_arrs,@,f_pixelgroup_get_dir_array,@,f_pixelgroup_move,@,f_initialize_groups,@,f_game_init,@,f_game_update,@,f_ball_update,@,f_get_at_coord,@,f_ball_update_x,@,f_ball_update_y,@,f_game_draw,@,f_zbtn,@,f_xbtn,@,f_ybtn,@,f_draw,@",f_zobj,f_zobj_set,f_zobj_eval,function(...)
...
end,function(_ENV,sx,sy,newcol)
...
if not grid[gy][(gx+1)%32]then add(array_r,p)end
...
f_zobj_set(group,"id,pl,xstore,0,ystore,0,jumpstore,0,jump,#,coyote,~c_no,update,~f_ball_update,update_x,~f_ball_update_x,update_y,~f_ball_update_y;")
group:register()
elseif group.col==7 then
f_zobj_set(group,"id,wall;")
group:register()
elseif group.col%16>0 then
f_zobj_set(group,"id,movablewall;")
group:register()
end
end
end,function(_ENV)
...
end,function(f,a)return f(a)and f(a+1)and 0 or f(a)and-1 or f(a+1)and 1 or 0 end,function()return f_zbtn(btn,0)end,function()return f_zbtn(btn,2)end,function(callback)
end)
g_zclass={}
